...
public class AuctionServiceImpl implements AuctionService {
    ...
    private Function<Auction, AuctionDto> getInformationAboutAuction() {
        return auction -> {
            ...
            try {
                ...
                final List<PhoneDto> phoneDtoList = CollectionUtils.isEmpty(auctioneer.getPhones()) ?
                        List.of() : phoneService.findPhonesByIds(
                        auctioneer.getPhones()
                                .stream()
                                .map(PhoneDto::getId)
                                .collect(Collectors.toList())
                );
    ...
    @Override
    public List<AuctionDto> getAllOpenedAuctions() {
        return this.getAllAuctions()
                .stream()
                .filter(auctionDto -> auctionDto.getStatus().equals("OP"))
                .collect(Collectors.toList());
    }
